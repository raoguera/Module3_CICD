...
mlflow.set_experiment("MLOps_CICD_experiment")     # complete the code to set the MLflow experiment name (same as the dev experimentation cell)
...
numeric_features = ['Age','DurationOfPitch','NumberOfPersonVisiting','NumberOfFollowups','NumberOfTrips','NumberOfChildrenVisiting','MonthlyIncome']   # complete the code to list all numerical feature names (same as in prep.py)

categorical_features = ['TypeofContact','CityTier','Occupation','Gender','ProductPitched','PreferredPropertyStar','MaritalStatus','Passport','PitchSatisfactionScore','OwnCar','Designation']   # complete the code to list all categorical feature names (same as in prep.py)
...
param_grid = {
    'xgbclassifier__n_estimators': [25,50,75],        # Number of boosting trees. More trees can improve performance but increase training time.
    ...
    'xgbclassifier__colsample_bytree': [.6,.8,1],    # Fraction of features sampled when building each tree.
    'xgbclassifier__colsample_bylevel': [.7,.9],   # Fraction of features sampled at each tree level.
...
with mlflow.start_run():
    ...
    model_path = "AML_MLOps_Assessment/Deployment/best_model.joblib"   # Specify the local file path (inside tourism_project/deployment/) where the trained model should be saved.
